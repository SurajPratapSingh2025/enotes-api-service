...
public class SwaggerConfig {
	...
	public OpenAPI openAPI() {
		...
		info.setTermsOfService("http://examenotes.com");
		info.setContact(new Contact().email("contact@examenotes.com").name("sps").url("http://examenotes.com"));
		info.setLicense(new License().name("Enotes 1.0").url("http://examenotes.com"));
		
		List<Server> serverList = List.of(
				new Server().description("Dev").url("http://localhost:9090"),
				new Server().description("Test").url("http://localhost:9091"),
				new Server().description("Prod").url("http://localhost:9092")
				);
		//bear sdbjkj.sdfdvs.sdfvsdc
